...
public class AgentServiceImpl implements AgentService {
	...
	public AgentInfoDto getAgentInfo(int userId) throws ServiceException {
		...
		try {
			...
			/*
			 * System.out.println("verifica que exista el usuario"); User
			 * userValidate = userEao.findById(userId);
			 */
			if (agentInfo == null) {
			...
			/*
			 * String campaignNames=""; for (int i=0;
			 * i<session.getCurrentSessionSessionType().getSessionsCampaign().
			 * size(); i++) { SessionCampaign sessionCampaign =
			 * session.getCurrentSessionSessionType().getSessionsCampaign().get(
			 * i); if (i==0) { campaignNames +=
			 * sessionCampaign.getCampaign().getName(); }else{ campaignNames +=
			 * ", "+sessionCampaign.getCampaign().getName(); } }
			 * agentInfoDto.setCampaignNames(campaignNames);
			 */

			String campaignNames = "";
			for (int i = 0; i < agentInfo.getCurrentSession().getCurrentSessionSessionType().getSessionsCampaign()
					.size(); i++) {
				SessionCampaign sessionCampaign = agentInfo.getCurrentSession().getCurrentSessionSessionType()
						.getSessionsCampaign().get(i);
				if (i == 0) {
			...
			agentInfoDto.setCampaignNames(campaignNames);
			agentInfoDto.setSessionType(
					agentInfo.getCurrentSession().getCurrentSessionSessionType().getSessionType().getName());
			...
			long sessionTypeTime = currentTime.getTime()
					- agentInfo.getCurrentSession().getCurrentSessionSessionType().getStartedAt().getTime();
	...
	@Override
	public List<SessionTypeEnum> findSessionTypes(int userId) throws ServiceException {
		try {
			...
			for (SessionCampaign sessionCampaign : agentInfo.getCurrentSession().getCurrentSessionSessionType()
					.getSessionsCampaign()) {
	...
	@Override
	public void changeSessionType(int userId, short sessionTypeSelected) throws ServiceException {
			...
			if (user.getCurrentSession().getCurrentSessionSessionType() != null) {
				...
				for (SessionCampaign sessionCampaign : user.getCurrentSession().getCurrentSessionSessionType()
					...
					if (sessionTypeEnum.equals(SessionTypeEnum.AVAILABLE)) {
						amiService.unpauseManyQueues(sessionCampaign.getSessionsQueue(),
								user.getCurrentSession().getCurrentSessionSessionType().getPeer().toString(),
								sessionCampaign.getCampaign().getServer());
					} else {
						amiService.pauseManyQueues(sessionCampaign.getSessionsQueue(),
								user.getCurrentSession().getCurrentSessionSessionType().getPeer().toString(),
								sessionCampaign.getCampaign().getServer());
					}
				}
	...
	public void openSessions2(User user, String peer) throws ServiceException {
		...
		try {
			...
			for (SessionCampaign sessionCampaign : newSession.getCurrentSessionSessionType().getSessionsCampaign()) {
				amiService.addManyQueues(sessionCampaign.getSessionsQueue(), peer,
	...
	public void closeSessions(User user) throws ServiceException {
		try {
			...
			if (currentSession.getCurrentSessionSessionType() != null) {
				...
				for (SessionCampaign sessionCampaign : currentSession.getCurrentSessionSessionType()
						.getSessionsCampaign()) {
					amiService.removeManyQueues(sessionCampaign.getSessionsQueue(), peerName.toString(),
							sessionCampaign.getCampaign().getServer());
				}
			}
	...
	public void restartAgent(int userId) throws ServiceException {
		try {
			...
			String host = "";
			Short peer = null;
			if (user.getCurrentSession() != null && user.getCurrentSession().getCurrentSessionSessionType() != null) {
				host = user.getCurrentSession().getCurrentSessionSessionType().getHost();
			...
			for (SessionCampaign sessionCampaign : user.getCurrentSession().getCurrentSessionSessionType()
					.getSessionsCampaign()) {
				amiService.removeManyQueues(sessionCampaign.getSessionsQueue(), peer.toString(),
						sessionCampaign.getCampaign().getServer());
			}
			...
			for (SessionCampaign sessionCampaign : user.getCurrentSession().getCurrentSessionSessionType()
					.getSessionsCampaign()) {
				amiService.addManyQueues(sessionCampaign.getSessionsQueue(), peer.toString(),
						sessionCampaign.getCampaign().getServer(), user.getSessionType().getIsPaused());
			}
	...
	public void loginAgent2(int userId, String anexo) throws ServiceException {
		try {
			...
			// VERIFICA QUE EL ANEXO ESTE DISPONIBLE
			/*
			 * String peerName = amiService.searchPeer(host,server); if
			 * (peerName == null) { throw new
			 * PeerNotFoundException(host,server.getName()); }
			 */
			String peerName = anexo;
